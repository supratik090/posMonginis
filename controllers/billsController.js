...
//              quantity: { $gt: 0 },
//              isActive: true,
//              returnDate: { $gte: new Date() }, // Includes today and future dates
//            }).sort({ createdAt: 1 });
//
...
const getTodaysReturns = async (req, res) => {
  try {
   ...
   const { returnDate } = req.query; console.info("got request:", returnDate);
   if (!returnDate)
    { return res.status(400).json({ error: "returnDate is required" }); }

    const baseDate = moment(returnDate, "YYYY-MM-DD").startOf("day");



    // Compute T-1, T-2, T-3 dates (same format as stored in DB)
    const dates = {
      t1: baseDate.clone().subtract(1, "days").format("DD/MM/YYYY"),
      t2: baseDate.clone().subtract(2, "days").format("DD/MM/YYYY"),
      t3: baseDate.clone().subtract(3, "days").format("DD/MM/YYYY"),
    };

    // Get all pastry items
    const items = await itemModel.find({ category: "Pastry" });
    ...
    const data = await Promise.all(
      items.map(async (item) => {
        ...
        for (let [key, dateStr] of Object.entries(dates)) {
          const inv = await inventoryModel.findOne({
            code: item.code,
            invoiceDate: dateStr, // exact match because it's stored as string
          });
          console.info("Inventory", dateStr, inv?.quantityOriginal);
          results[key] = inv?.quantityOriginal ?? 0;
        }
    ...
    res.json({ returnDate, data });
  } catch (err) {
    console.error("Error in /pastry-returns:", err);
